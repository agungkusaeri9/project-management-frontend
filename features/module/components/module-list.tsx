...
export function ModuleList({ projectId }: ModuleListProps) {
  ...
  const handleCreate = () => {
    ...
    createModule(
      { project_id: projectId, name: newName.trim(), sort_order: modules.length + 1 },
      {
        onSuccess: () => {
  ...
  const handleUpdate = (id: string, module: Module) => {
    if (!editName.trim()) return;
    updateModule(
      { id, data: { ...module, name: editName.trim() } },
      {
        onSuccess: () => {
    ...
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
        <ul className="divide-y divide-slate-100 dark:divide-slate-800/50">
          {modules.map((module) => (
            <li key={module.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex items-center justify-between group">
              {editingId === module.id ? (
                <div className="flex-1 flex items-center gap-2 mr-4">
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleUpdate(module.id, module)}
                    ...
                    autoFocus
                  />
                  <button onClick={() => handleUpdate(module.id, module)} className="p-1.5 text-green-600 hover:bg-green-100 dark:hover:bg-green-900/30 rounded-md">
                ...
                <div className="flex-1">
                  <h3 className="font-medium text-slate-900 dark:text-slate-100">{module.name}</h3>
                </div>
              )}
              ...
              {editingId !== module.id && (
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    onClick={() => { setEditingId(module.id); setEditName(module.name); }}
          ...
          {isAdding ? (
            <li className="p-4 bg-indigo-50/30 dark:bg-indigo-500/5 border-l-2 border-indigo-500 flex items-center gap-2">
              <input
                type="text"
                placeholder="Type module name and press Enter to save..."
                value={newName}
