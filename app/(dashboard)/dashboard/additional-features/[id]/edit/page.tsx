...
import { useAdditionalFeature, useUpdateAdditionalFeature } from '@/features/additional-feature/hooks/use-additional-features';
...
export default function EditAdditionalFeaturePage() {
  const router = useRouter();
  const params = useParams();
  const id = params.id as string;

  const { data: feature, isLoading: isLoadingFeature } = useAdditionalFeature(id);
  const { data: projects, isLoading: isLoadingProjects } = useProjects();
  const { mutate: updateFeature, isPending } = useUpdateAdditionalFeature();

  const { register, control, handleSubmit, formState: { errors }, watch, setValue, reset } = useForm<FormValues>({
    defaultValues: {
      project_id: '',
      status: 'ongoing',
  ...
  // Populate form when feature is loaded
  useEffect(() => {
    if (feature) {
      reset({
        project_id: feature.project_id,
        status: feature.status || 'ongoing',
        start_date: feature.start_date ? new Date(feature.start_date).toISOString().split('T')[0] : '',
        estimated_completion_date: feature.estimated_completion_date ? new Date(feature.estimated_completion_date).toISOString().split('T')[0] : '',
        items: feature.items?.map(item => ({
          id: item.id,
          name: item.name,
          description: item.description || '',
          estimated_completion_date: item.estimated_completion_date ? new Date(item.estimated_completion_date).toISOString().split('T')[0] : '',
  ...
  // Watch selected project to display customer info
  ...
  useEffect(() => {
    // Only auto-calc if items exist and we are actively manipulating them
    if (items && items.length > 0) {
      ...
      if (dates.length > 0) {
        ...
        // Prevent infinite re-renders or unwanted overwrites by checking current value
        // Note: react-hook-form's getValues() could be used, but watching and checking prevents loops if used carefully.
        // It's usually fine as long as we only set it if it changes.
      }
    }
  }, [items]); // Simplified to avoid overwriting user edits on initial load
  ...
  const handleAddItem = () => {
    ...
    // Manually trigger recalculation of max date upon adding/editing
    recalculateMaxDate();
  };
  ...
  const onSubmit = (data: FormValues) => {
    ...
    updateFeature({ id: feature.id, data: payload }, {
      onSuccess: () => {
        toast.success('Additional feature updated successfully!');
        router.push(`/dashboard/additional-features`);
      },
      onError: (err: any) => {
        toast.error(err.response?.data?.error || 'Failed to update additional feature');
      }
    });
  };

  if (isLoadingProjects || isLoadingFeature) {
    return (
    ...
    <div className="w-full space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Edit Additional Feature</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Update project assignment and feature items.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="lg:col-span-2 flex flex-col justify-center bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
              ...
              {selectedProject ? (
                <div className="font-medium text-slate-900 dark:text-slate-100">
                  {selectedProject.customer_name || 'No Customer Attached'}
                </div>
              ) : (
                <div className="text-sm text-slate-400 italic">Select a project first</div>
              )}
            </div>
