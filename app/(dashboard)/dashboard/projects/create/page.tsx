...
import { projectMemberService } from '@/features/project/services/project-member.service';
...
export default function CreateProjectPage() {
  ...
  const [salesTeam, setSalesTeam] = useState<SalesRow[]>([{ user_id: '', is_pic: false }]);
  ...
  // Global PIC enforcement: clears all other is_pic flags across all sections
  const setPIC = (section: 'programmer' | 'electrical' | 'sales', idx: number, checked: boolean) => {
    const clearOthers = (rows: any[]) => rows.map((r) => ({ ...r, is_pic: false }));
    if (!checked) {
      // Just toggle off
    ...
    setProgrammers((p) => clearOthers(p));
    setElectricals((e) => clearOthers(e));
    setSalesTeam((s) => clearOthers(s));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      status, customer_id: customerId || null,
    ...
    createProject(payload as any, {
      onSuccess: async (res: any) => {
        ...
        if (newId) {
          const members: any[] = [];
          for (const p of programmers) if (p.user_id) members.push({ user_id: p.user_id, role: 'programmer', sub_role: p.sub_role || null, is_pic: p.is_pic });
          for (const e of electricals) if (e.user_id) members.push({ user_id: e.user_id, role: 'electrical_engineer', sub_role: e.sub_role || null, is_pic: e.is_pic });
          for (const s of salesTeam) if (s.user_id) members.push({ user_id: s.user_id, role: 'sales', sub_role: null, is_pic: s.is_pic });
          if (members.length > 0) try { await projectMemberService.sync(newId, members); } catch { }
        }
  ...
  const MemberSection = ({
    title, icon, iconColor, addLabel, rows, subRoles, onAdd, onRemove, onUserChange, onSubRoleChange, onPICChange, userOptions, emptyLabel,
  }: any) => (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-3">
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
        ...
        <button type="button" onClick={onAdd}
          className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold ${iconColor} rounded-xl transition-colors`}>
        ...
        <div className="space-y-2">
          {rows.map((row: any, i: number) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <PICToggle checked={row.is_pic} onChange={(v) => onPICChange(i, v)} />
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/dashboard/projects" className="p-2 -ml-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Briefcase className="w-6 h-6 text-indigo-600 dark:text-indigo-400" /> Create Project
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-0.5 text-sm">Fill in project details and assign team members.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 space-y-5">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
              ...
              <div>
                ...
                <input type="text" value={code} onChange={(e) => setCode(e.target.value)} placeholder="PRJ-001" className={inputClass} required />
              </div>
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="My Awesome Project" className={inputClass} required />
              </div>
              <div>
          ...
          <div className="md:col-span-2 space-y-4">
            <MemberSection
              ...
              userOptions={programmerUsers} emptyLabel="No programmers added yet."
            ...
            <MemberSection
              ...
              userOptions={electricalUsers} emptyLabel="No electrical engineers added yet."
                ...
                <div className="space-y-2">
                  {salesTeam.map((row, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <PICToggle checked={row.is_pic} onChange={(v) => setPIC('sales', i, v)} />
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <Link href="/dashboard/projects" className="px-5 py-2.5 rounded-xl font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-sm">Cancel</Link>
